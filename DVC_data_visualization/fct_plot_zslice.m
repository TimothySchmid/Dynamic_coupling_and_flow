function fct_plot_zslice(INPUT, CELL)
...
for itime = 1:1:l
    ...
    plt_vcoord_2d = squeeze(X(:,pos,:));
    
    % get coordinates for vectors
    plt_hcoord_vec_2d = squeeze(Z(1:incr:end,pos,1:incr:end));
    plt_vcoord_vec_2d = squeeze(X(1:incr:end,pos,1:incr:end));
    ...
    plt_vec_v2d = double(squeeze(CELL.disp.X{itime}(:,pos,:)));
    ...
    hAx.FontSize = 14;
    axis equal
    axis([zcoords(1) zcoords(end) xcoords(1) xcoords(end)])
